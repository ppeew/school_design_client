...
export default {
    ...
    returnMessage(that, status = false) {
        ...
        let url = apiUrl+"/api/v1/message?userId="+JSON.parse(sessionStorage.getItem("userInfo")).id
    ...
    logOut(that) {
        ...
        // 如果您希望在一段时间后执行一些操作，可以使用 setTimeout
        setTimeout(() => {
            // 清除用户信息等操作
            sessionStorage.removeItem("userInfo");
            this.setUserInfoAndLoginTypeToStore(that);
