    ...
    export default {
        ...
        methods: {
            ...
            haveRead:function(id,i){
                let url = apiUrl+"/api/v1/message/" + id
                const that = this
                service.put(url).then(function (res) {
            ...
            allHaveRead:function(){
                let url = apiUrl+"/api/v1/message/all?userId="+JSON.parse(sessionStorage.getItem("userInfo")).id
                const that = this
                service.put(url).then(function (res) {
                    if (res.data.code === 200){
            ...
            deleteNotice:function(id,i){
                let url = apiUrl+"/api/v1/message?id="+id
                const that = this
                service.delete(url).then(function (res) {
                    if (res.data.code === 200){
            ...
            deleteAllNotice:function(){
                this.$msgbox.confirm('确定删除所有通知吗, 是否继续?', '提示', {
                    ...
                    let url = apiUrl+"/api/v1/message/delete_all?userId="+JSON.parse(sessionStorage.getItem("userInfo")).id
                    const that = this
                    service.delete(url).then(function (res) {
                        if (res.data.code === 200){
                            that.$util.returnMessage(that);
