                ...
                <template v-else>
                    <div class="post-meta wrapper-lg" v-for="(item , i ) in blogInfos ">
                        ...
                        <p class="summary l-h-2x text-muted blog_info">
                            ...
                            <span class="m-r-sm">&nbsp;{{item.BlogTime}}</span>
    ...
    export default {
        ...
        methods: {
            getBlogInfoPage: function () {
                ...
                service({
                    method: "get",
                    url: apiUrl+"/api/v1/collect?pageSize="+that.pageInfo.pageSize+"&pageIndex="+that.pageInfo.currentPage+"&id="+JSON.parse(sessionStorage.getItem("userInfo")).id
                }).then(res => {
                    if(res.data.code === 200){
                        that.blogInfos = res.data.data.list
                        that.$Loading.finish()
                        that.pageInfo.total = 1
                    }
                })
            },
