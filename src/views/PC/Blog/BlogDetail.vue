            ...
            <template v-else>
                ...
                <a-comment v-for="(item,i) in commentInfo">
                    ...
                    <a-avatar
                            ...
                            :src="item.UserImage"
                    />
                    <p slot="content" v-html="item.Content">
                    </p>
                    <a-tooltip :title="moment(item.CommentTime).format('YYYY-MM-DD HH:mm:ss')" slot="datetime">
                        <span>{{item.CommentTime}}</span>
                    </a-tooltip>

                    <!-- 二级评论区-->
                    <a-comment v-if="item.CommentResp.length !== 0" v-for="(itemSec,i) in item.CommentResp">
        ...
        methods: {
            ...
            doBlogConnect:function(){
                ...
                if (!this.isBlogConnect) {
                    url = apiUrl+"/api/v1/collect"
                }else{
                    url = apiUrl+"/api/v1/removeCollect"
                }
                const that = this
                service({
                    method:'post',
                    data:{
                        "blogId":that.blogInfo.id.toString(),
            ...
            doBlogGood:function(){
                ...
                if (!this.isBlogGood) {
                    //点赞
                    url = apiUrl+"/api/v1/blogs/" + this.blogInfo.id.toString()+"?type=1"
                }else{
                    return
                }
                const that = this
            ...
            secCommentAction:function(comment){
                this.secondPreview = "回复：<strong>@" + comment.UserName + "</strong> &nbsp;" + comment.Content;
                this.isSecond = true;
                console.log(comment)
                this.secondCommentParentId = comment.ID
                        ...
                        if (this.isSecond){
                            service({
                                ...
                                data:{
                                    ...
                                    targetId: that.secondCommentParentId.toString(),
                                    content:that.commentHtml
                                }
                            }).then(res => {
                                if (res.data.code === 200){
                                    ...
                                    that.isSecond = false
                                    that.getBlogInfoAndCommentsById()
            ...
            getBlogCommonById:function(){
                let url = apiUrl+"/api/v1/comments?blogId=" + this.blogId
                const that = this
            ...
            getBlogInfoById: function () {
                let url = apiUrl+"/api/v1/blogs/" + this.blogId
                const that = this
                service.get(url).then(function (res) {
                    that.blogInfo = res.data.data
                    that.content = marked(that.blogInfo.msg)
            ...
            getBlogInfoAndCommentsById:function(){
                ...
                Promise.all([
                   ...
                   new Promise((resolve, reject)=>{
                       ...
                       service.get(url).then(function (res) {
                           if (res.data.code === 200){
                               that.commentInfo = res.data.data.list
                           }else{
                               reject(res.data.code)
